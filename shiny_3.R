...
ui <- fluidPage(
  ...
  navbarPage(
    id = "main_nav",
    title = div(class = "navbar-brand", "US Census Data Analysis Project (Group C)"),
    tabPanel("🏠",
             tabsetPanel(
...
server <- function(input, output, session) {
  ...
  output$poverty <- renderUI({
    if (active_tab_2() == "poverty") {
      tags$iframe(src = "poverty.html", style = "width:100%; height:800px;")
    }
  })
  ...
  output$migration <- renderUI({
    if (active_tab_3() == "migration") {
      tags$iframe(src = "migration.html", style = "width:100%; height:800px;")
    }
  })
  ...
  output$ev <- renderUI({
    if (active_tab_4() == "ev") {
      tags$iframe(src = "ev.html", style = "width:100%; height:800px;")
    }
  })
